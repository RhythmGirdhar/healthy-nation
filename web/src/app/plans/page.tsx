...
export default function PlansPage() {
  ...
  return <>
    ...
    <section className="weekly-section"><div className="container">
      <div className="section-intro"><div><span className="eyebrow">Meal rotation</span><h2>{catalog.weeklyMenu.isSample ? "Sample weekly menu" : "Weekly menu"}</h2><p>{catalog.weeklyMenu.label}</p>{catalog.weeklyMenu.isSample && <p className="small-note">An example rotation, not scheduled or available meals.</p>}</div></div>
      <div className="weekly-grid">{catalog.weeklyMenu.days.map((day) => {
        const meal = catalog.meals.find((entry) => entry.id === day.mealId);
        return meal ? <article className="week-day" key={day.day}><span className="eyebrow">{day.day}</span><Image src={meal.image.src} alt="" width={180} height={149} unoptimized /><div><h3>{meal.name}</h3><Link href={`/menu/${meal.slug}/`} className="text-link">Explore meal <span aria-hidden="true">↗</span></Link></div></article> : null;
      })}</div>
    </div></section>
